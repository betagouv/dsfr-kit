...
const meta: Meta = {
  ...
  render: (args) => {
    ...
    const noIcon =
      args.displayType === "status" ? args.hasNoIcon : !args.hasIcon;
    const finalIcon =
...
const renders = (argsArray: Record<string, unknown>[]) =>
  argsArray.map((args) =>
    // biome-ignore lint/suspicious/noExplicitAny: Storybook types are complex
    // biome-ignore lint/style/noNonNullAssertion: Render function is present
    meta.render!({ ...meta.args, ...args }, {} as any),
  );

...
function detectEventListener(
  ...
  if (
    ...
    node.expression.name.text.startsWith("on") &&
    node.arguments.length === 0
  ) {
    const eventType = node.expression.name.text.slice(2); // Remove 'on' prefix
    const target = getObjectFromPropertyAccess(node.expression, sourceFile);
...
function detectDomManipulation(
  ...
  if (
    ...
    if (isPropertyAccess(node.parent.left)) {
      const propertyName = node.parent.left.name.text;
      const target = getObjectFromPropertyAccess(node.parent.left, sourceFile);
      const value = getNodeText(node.parent.right, sourceFile);
      ...
      if (
        ...
        return {
          ...
          value,
          lineNumber: getLineNumber(node.parent, sourceFile),
        };
      }
    }
  }
